...
set   :repository,          "ssh://git@bitbucket.org:jeloibit/middbites.git"
...
namespace :deploy do
  ...
  namespace :db do
    ...
    task :sync_yaml do
      ...
      system "rsync -vr -e 'ssh -p 1026' --exclude='.DS_Store' config/database.yml #{user}@#{application}:#{shared_path}/config/"
    end
  ...
  namespace :nginx do
    ...
    task :create do
      ...
      system "rsync -vr -e 'ssh -p 1026' tmp/#{domain} #{user}@#{application}:/opt/nginx/conf/sites-enabled/#{domain}"
      File.delete("tmp/#{domain}")
    ...
    desc "Restarts NginX."
    task :restart do
      # runner "service nginx stop", :sudo => true
